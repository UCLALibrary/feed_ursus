...
import feed_ursus.year_parser as year_parser
import feed_ursus.date_parser as date_parser
    ...
    def load_csvs(self, filenames: typing.List[str]):
        ...
        for row in csv_data.values():
            if row.get("Object Type") == "Collection":
                ursus_id = row["Item ARK"].replace("ark:/", "").replace("/", "-")[::-1]
                self.collection_names[ursus_id] = row["Title"]
